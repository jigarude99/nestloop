...
export async function fetchExpenses(householdId: string): Promise<Expense[]> {
  ...
  const { data, error } = await supabase
    .from("expenses")
    .select(
      "id,title,merchant,category,amount_cents,paid_by,purchased_at,note,receipt_path,created_at," +
...
export async function completeRotation(rotation: Rotation): Promise<void> {
  const supabase = getSupabase();
  if (!rotation.queue.length) return;
  const personId = rotation.queue[rotation.currentIndex];
  const nextIndex = (rotation.currentIndex + 1) % rotation.queue.length;
...
export async function fetchSlots(householdId: string): Promise<ScheduleSlot[]> {
  ...
  return (data ?? []).map((row: any) => ({
    ...
    start: fmtTime(row.starts_at),
    end: fmtTime(row.ends_at),
